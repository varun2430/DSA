// Given two strings s and t, return the number of distinct subsequences of s which equals t.
// The test cases are generated so that the answer fits on a 32-bit signed integer.

// Example:
// Input:  s = "babgbag", t = "bag"
// Output: 5

#include <bits/stdc++.h>
using namespace std;

int solve_m(int i, int j, string &s1, string &s2, vector<vector<int>> &dp)
{
    // we found the required string in given string
    if (j < 0)
    {
        return 1;
    }

    // we exausted the given string
    if (i < 0)
    {
        return 0;
    }

    if (dp[i][j] != -1)
    {
        return dp[i][j];
    }

    if (s1[i] == s2[j])
    {
        dp[i][j] = solve_m(i - 1, j - 1, s1, s2, dp) + solve_m(i - 1, j, s1, s2, dp);
        return dp[i][j];
    }

    dp[i][j] = solve_m(i - 1, j, s1, s2, dp);
    return dp[i][j];
}

// for tabulation we shift the index right by 1 to deal with -ve index
// i.e. 0 -> -1 && n -> n-1
int solve_t(int &n, int &m, string &s1, string &s2)
{
    vector<vector<double>> dp(n + 1, vector<double>(m + 1, 0));

    for (int i = 0; i <= n; i++)
    {
        dp[i][0] = 1;
    }

    for (int j = 1; j <= m; j++)
    {
        dp[0][j] = 0;
    }

    for (int i = 1; i <= n; i++)
    {
        for (int j = 1; j <= m; j++)
        {
            if (s1[i - 1] == s2[j - 1])
            {
                dp[i][j] = dp[i - 1][j - 1] + dp[i - 1][j];
            }
            else
            {
                dp[i][j] = dp[i - 1][j];
            }
        }
    }

    return (int)(dp[n][m]);
}

int solve_so(int &n, int &m, string &s1, string &s2)
{
    vector<double> prev(m + 1), curr(m + 1);

    prev[0] = 1;
    curr[0] = 1;
    for (int j = 1; j <= m; j++)
    {
        prev[j] = 0;
        curr[j] = 0;
    }

    for (int i = 1; i <= n; i++)
    {
        for (int j = 1; j <= m; j++)
        {
            if (s1[i - 1] == s2[j - 1])
            {
                curr[j] = prev[j - 1] + prev[j];
            }
            else
            {
                curr[j] = prev[j];
            }
        }

        prev = curr;
    }

    return (int)(prev[m]);
}

int numDistinct(string s, string t)
{
    int n = s.length();
    int m = t.length();

    // vector<vector<int>> dp(n, vector<int>(m, -1));
    // return solve_m(n-1, m-1, s, t, dp);

    return solve_so(n, m, s, t);
}
